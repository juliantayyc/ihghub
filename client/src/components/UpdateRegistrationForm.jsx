...
const UpdateRegistrationForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'matriculationNumber' && /^[A].{8}$/.test(value)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[A].{8}$/.test(formData.matriculationNumber)) {
      setMatriculationNumberError(
        'Matriculation number must start with "A" and be exactly 9 characters long.'
      );
      return;
    }
    try {
